...
class TasksTable extends Table
{
    ...
    public function validationDefault(Validator $validator): Validator
    {
        ...
        $validator
            ->scalar('taskname')
            ->maxLength('taskname', 255)
            ->requirePresence('taskname', 'create')
            ->notEmptyString('taskname');

        $validator
            ->integer('pomodoro_count')
            ->requirePresence('pomodoro_count', 'create')
            ->notEmptyString('pomodoro_count');

        $validator
            ->integer('importance_level')
            ->requirePresence('importance_level', 'create')
            ->notEmptyString('importance_level');

        $validator
            ->integer('urgency_level')
            ->requirePresence('urgency_level', 'create')
            ->notEmptyString('urgency_level');
